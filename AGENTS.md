...
- **Channels:** only `eve` (route-auth channel for chat). GitHub and Linear channels are not used. The github extension remains on the root for opening draft PRs; approvals park on chat.
